...
const ReportGenerator = ({ csvData }: Props) => {
  ...
  const generatePDF = () => {
    ...
    const incomeRows = rows.filter((row) => row[subCategoryIndex] === "Income");
    ...
    doc.setFontSize(16);
    doc.setTextColor(isProfit ? 0 : 200, isProfit ? 150 : 0, isProfit ? 0 : 0); // green or red
    doc.text(`${isProfit ? "Profit" : "Loss"}: RM ${profitLoss}`, 14, summaryY);
    ...
    const assetRows = rows.filter((row) => row[subCategoryIndex] === "Assets");
